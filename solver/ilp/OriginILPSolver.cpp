...
#include <gurobi_c++.h>
#include <cmath>
#include "OriginILPSolver.h"

OriginILPSolver::OriginILPSolver(uint32_t threadCount, bool printOutput, bool usePresolve) : threadCount(threadCount),
                                                                                             printOutput(printOutput),
                                                                                             usePresolve(usePresolve) {}

BinaryILPSolution OriginILPSolver::solve(const SchedulingProblem &schedProblem) const {
    return solve_impl(schedProblem, BinaryILPSolution());
}

BinaryILPSolution OriginILPSolver::solve_impl(const SchedulingProblem &schedProblem, const BinaryILPSolution &bestSolution) const {
    if (schedProblem.empty()) {
        std::cerr << "[OriginILPSolver::solve_impl]: schedProblem is empty" << std::endl;
    ...
    try {
        ...
        if(!bestSolution.empty()) {
            model.set(GRB_DoubleParam_Cutoff, (double)(bestSolution.getObjective() + 1));
        }
    ...
    if (model.get(GRB_IntAttr_Status) == GRB_CUTOFF) {
        return bestSolution;
    } else if (model.get(GRB_IntAttr_Status) != GRB_OPTIMAL) {
        std::cerr << "[OriginILPSolver::solve_impl]: not optimal solution found" << std::endl;
        return BinaryILPSolution();
    }
...
BinaryILPSolution OriginILPSolver::solve(const SchedulingProblem &schedProblem, const BinaryILPSolution &lowerBound) const {
