...
#include <gurobi_c++.h>
#include <cmath>
#include "GurobiMaxProcessingTimeILPSolverMulti.h"

GurobiMaxProcessingTimeILPSolverMulti::GurobiMaxProcessingTimeILPSolverMulti(uint32_t threadCount, bool printOutput, bool usePresolve) : threadCount(threadCount),
                                                                                                                               printOutput(printOutput),
                                                                                                                               usePresolve(usePresolve) {}

BinaryILPSolution GurobiMaxProcessingTimeILPSolverMulti::solve(const SchedulingProblem &schedProblem) const {
    return solve_impl(schedProblem, BinaryILPSolution());
}

BinaryILPSolution GurobiMaxProcessingTimeILPSolverMulti::solve_impl(const SchedulingProblem &schedProblem, const BinaryILPSolution &lowerBound) const {
    if (schedProblem.empty()) {
        std::cerr << "[GurobiMaxProcessingTimeILPSolverMulti::solve_impl]: schedProblem is empty" << std::endl;
    ...
    try {
        ...
        GRBLinExpr secondObjective;

        for (size_t i = 0; i < schedProblem.size(); ++i) {
            secondObjective += xVar[i];
        }

        model.setObjectiveN(secondObjective, 1);
...
BinaryILPSolution GurobiMaxProcessingTimeILPSolverMulti::solve(const SchedulingProblem &schedProblem, const BinaryILPSolution &lowerBound) const {
