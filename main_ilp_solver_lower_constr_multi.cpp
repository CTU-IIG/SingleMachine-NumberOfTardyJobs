...
#include "solver/relaxation/GurobiMaxProcessingTimeILPSolverMulti.h"
...
int main(int argc, char **argv) {
    ...
    try {
        ...
        MCFClassLPSolver classLpSolver(MCFClassSolver::RELAX_IV);
        LPSolution classLpSolution = classLpSolver.solve(schedulingProblem);

        ImprovedLowerBound lowerBoundHeuristic(ILPSolverImpl::IMPROVED, LowerBoundImpl::IMPROVED, UpperBoundImpl::IMPROVED, 8);
        BinaryILPSolution lowerBoundHSolution = lowerBoundHeuristic.solve(schedulingProblem, classLpSolution);

        GurobiMaxProcessingTimeILPSolverMulti originIlpSolver(threadLimit, false, false);
        BinaryILPSolution solution = originIlpSolver.solve(schedulingProblem, lowerBoundHSolution);
